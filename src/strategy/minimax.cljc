...
(defn- evaluate-state [game]
  (let [current-player (:current-player game)
    ...
    (println "State eval:" current-player evaluation)
    evaluation))
...
(defn- order-moves [game moves]
  (let [ordered-moves (sort-by (fn [move]
                               ...
                               moves)]
    ;; (println "Ordered moves:" (map :to ordered-moves))
...
(defn- adaptive-depth [game base-depth]
  (let [pieces-left (+ (get-in game [:players :A :in-hand])
    ...
    (println (str "Adaptive depth: " depth " (base: " base-depth ", pieces left: " pieces-left ")"))
    depth))
...
(defn- minimax [game depth maximizing? alpha beta]
  (if (or (zero? depth) (= :end-game (:state game)))
    (let [value (evaluate-state game)]
      (println "Leaf node reached. Value:" value)
      [value nil])
    (let [moves (order-moves game (safe-get-moves game))
      ...
      (if (empty? moves)
        (let [value (evaluate-state game)]
          (println "No moves available. Value:" value)
          [value nil])
        (loop [idx 1
          ...
          (if-not move
            (do
              (println "Finished evaluating all moves at depth" depth ". Best score:" best-score)
              [best-score best-move])
            (let [_ (println "Eval " idx "depth" depth)
              ...
              (println "Move" move "evaluated. Score:" score)
              (if (<= beta alpha)
                (do
                  (println "Pruning at depth" depth)
...
(defn select-move [game]
  ...
  (when (seq (engine/get-possible-moves game))
    (let [base-depth (get-in game [:strategy :params :depth] 3)
      ...
      (println "Selected move:" best-move "with score" score)
      best-move)))
